# Resumo da Matéria: Desenvolvimento Back-end
## UA 1 - Arquitetura - Aula 01: Arquitetura de uma aplicação Web e o formato JSON

> **Curso:** Gestão da TI | **Período:** 3º Período  
> **Módulo:** Modelagem e Segurança da Informação  
> **Disciplina:** Desenvolvimento Back-end  
> **Unidade:** UA 1 - Arquitetura  
> **Aula:** 01 - Arquitetura de uma aplicação Web e o formato JSON  
> **Carga de Vídeo:** ~29 min 43 s (4 blocos de vídeo)

---

## 📂 Checklist de Materiais Baixados
- [ ] `PDF da Matéria`: `Arquitetura de uma aplicação Web e o formato JSON.pdf`
- [ ] `PDF de Slides`: `Slides - Arquitetura de uma aplicação Web e o formato JSON.pdf`
- [ ] `Áudios`: MP3 das videoaulas
- [ ] `Legendas/Transcrições`: VTT / SRT / TXT
- [ ] `Resumo Gerado`: Markdown pronto para revisão

---

## 🎬 Blocos de Videoaulas
- **Parte 01:** Arquitetura de Uma Aplicação Web e o Formato Json (05:38) - *Prof. Denis Gonçalves Cople*
- **Parte 02:** Arquitetura de Uma Aplicação Web e o Formato Json II (08:29) - *Prof. Denis Gonçalves Cople*
- **Parte 03:** Arquitetura de Uma Aplicação Web e o Formato Json III (08:55) - *Prof. Denis Gonçalves Cople*
- **Parte 04:** Arquitetura de Uma Aplicação Web e o Formato Json IV (06:41) - *Prof. Denis Gonçalves Cople*

---

## 🎯 Objetivos de Aprendizagem
- [ ] Compreender os conceitos principais de **Arquitetura de uma aplicação Web e o formato JSON**.
- [ ] Conectar os fundamentos com as aplicações práticas em **Desenvolvimento Back-end**.
- [ ] Consolidar pontos críticos para exames e questionários (Quiz / Check de Aprendizagem).

---

## 📝 Resumo Consolidado (Para NotebookLM / Gemini)

### 1. Visão Geral e Conceitos Fundamentais
*Insira aqui o resumo gerado pelo Gemini / NotebookLM ou suas anotações principais.*

- **Conceito Chave 1:** 
- **Conceito Chave 2:** 
- **Conceito Chave 3:** 

### 2. Tópicos Abordados
*Resumo detalhado dos blocos de vídeo e do livro-texto.*

### 3. Tabela de Termos Técnicos, Siglas e Protocolos
| Termo / Sigla | Definição / Significado | Aplicação Prática |
| :--- | :--- | :--- |
| | | |
| | | |

---

## 🧠 Mapa Mental / Diagrama de Fluxo (Mermaid)
```mermaid
flowchart TD
    A["Arquitetura de uma aplicação Web e o formato JSON"] --> B["Conceito Principal"]
    A --> C["Tecnologias / Modelos"]
    B --> D["Funcionamento Prático"]
    C --> E["Boas Práticas & Segurança"]
```

---

## ❓ Perguntas de Fixação & Flashcards (Estudo Ativo)
1. **Pergunta:** 
   - **Resposta:** 
2. **Pergunta:** 
   - **Resposta:** 

---

## 💡 Dicas de Estudo
- Faça o **Check de Aprendizagem** e o **Quiz da UA** após revisar este resumo.
- Para gerar novas perguntas e simular testes, carregue este arquivo + o PDF da matéria no **NotebookLM** ou **Gemini**.
